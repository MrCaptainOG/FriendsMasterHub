...
export default function Shop() {
  ...
  const { mutate: buyItem } = useBuyItem({
    mutation: {
      ...
      onError: (err: { response?: { data?: { error?: string } } }, vars) => {
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-5">
          {items.map((item) => {
            ...
            return (
              <div
                key={item._id}
                className="bg-card border border-border rounded-xl overflow-hidden shadow-md hover:shadow-xl hover:border-primary/20 transition-all flex flex-col"
              >
                {item.imageUrl ? (
